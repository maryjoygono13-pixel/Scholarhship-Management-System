...
function renewalStatusLabel(r: any): string {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const semesterFilter = document.getElementById("semesterFilter") as HTMLSelectElement | null;
  const schoolYearFilter = document.getElementById("SchoolYearFilter") as HTMLSelectElement | null;
  const scholarshipFilter = document.getElementById("scholarshipFilter") as HTMLSelectElement | null;
  const programFilter = document.getElementById("programFilter") as HTMLSelectElement | null;
  ...
  async function loadLedger(): Promise<void> {
    try {
      const res = await fetch("api/list_renewal.php");
      const json = await res.json();
      if (json.success) {
        ledgerData = json.data;
        fillLedgerFilters();
        if (json.summary) {
          if (countPending) countPending.textContent = String(json.summary.pending ?? 0);
          if (countEligible) countEligible.textContent = String(json.summary.eligible);
          if (countAtRisk) countAtRisk.textContent = String(json.summary.at_risk);
          if (countTerminated) countTerminated.textContent = String(json.summary.terminated);
          if (countTotal) countTotal.textContent = String(json.summary.total);
        }
        renderLedger();
      }
    } catch (e) {
  ...
  function fillLedgerFilters(): void {
    const fill = (sel: HTMLSelectElement | null, placeholder: string, values: string[]): void => {
      if (!sel) return;
    ...
    fill(schoolYearFilter, "School Year", ledgerData.map((r: any) => r.schoolYear));
    fill(scholarshipFilter, "Scholarship Type", ledgerData.map((r: any) => r.scholarshipType));
    fill(programFilter, "Program", ledgerData.map((r: any) => r.programCode));
  }
  ...
  function renderLedger(): void {
    ...
    const filtered = ledgerData.filter((r: any) => {
  ...
  window.openEvalModal = function(id: number): void {
    selectedRecord = ledgerData.find((r: any) => r.id === id);
    if (!selectedRecord || !modalOverlay) return;

    if (modalName) modalName.textContent = selectedRecord.name;
    if (modalId) modalId.textContent = `Student ID: ${selectedRecord.studentId}`;
    if (modalSeal) {
  ...
  window.editRenewal = function(event: MouseEvent, id: number): void {
    event.stopPropagation();
    if (window.openEvalModal) window.openEvalModal(id);
  };

  window.confirmDeleteRenewal = function(event: MouseEvent, id: number): void {
    event.stopPropagation();
    const item = ledgerData.find((r: any) => r.id === id);
    if (!item) return;
    deletingRenId = id;
    if (renDeleteTarget) renDeleteTarget.textContent = item.name;
    if (renDeleteOverlay) renDeleteOverlay.classList.add("open");
  };

  function closeDeleteModal(): void {
    if (renDeleteOverlay) renDeleteOverlay.classList.remove("open");
    deletingRenId = null;
  }
  ...
  async function updateStatus(action: string): Promise<void> {
    ...
    try {
      const res = await fetch("api/list_renewal.php", { method: "POST", body: formData });
      const json = await res.json();
      if (json.success) {
  ...
  if (renDeleteConfirmBtn) {
    renDeleteConfirmBtn.addEventListener("click", async () => {
      if (!deletingRenId) return;
      try {
        const res = await fetch(`api/delete_renewal.php?id=${deletingRenId}`, { method: "POST" });
        const json = await res.json();
        if (json.success) {
          closeDeleteModal();
          loadLedger();
  ...
  function exportRenewalToCsv(): void {
  ...
  if (exportRenewalBtn) exportRenewalBtn.addEventListener("click", exportRenewalToCsv);
